...
## MCP Tools
MaxTAC includes a small Python MCP server declared in `.mcp.json`. When enabled in Codex, it exposes deterministic tools for workspace setup and status, phase management, research submodules, ledger operations, auditor/debater prompt persistence, debate tallying, SAST packet validation, and evidence packing. `workspace_status` also reports research hygiene and attention-lock warnings so long-running goals get a mechanical checkpoint before context momentum takes over. It also wraps the existing debugger, fuzzing, LPAC, IPSW, Ghidra, JADX, radare2, and RE-readiness helper scripts so agents can call the same evidence collectors through MCP while preserving their JSON and markdown artifacts.
