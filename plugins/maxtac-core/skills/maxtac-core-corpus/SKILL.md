...
description: "Use this skill when MaxTAC research needs a growable faceted knowledge base, canonical research notes, closure notes, false-negative review references, tag and graph based retrieval, anti-tunnel orientation packs, generated research views, import of existing research markdown, or workspace corpus hygiene instead of hand-growing directory hierarchies."
---
...
For broad negative conclusions, long no-finding sessions, or many thin closures used together, point the closure note at the `contracts/false-negative-reviews/<id>/review.json` or `review.md` artifact. The note body must preserve the weaker claim: "no reportable chain survived the recorded evidence," plus false-negative risk, key missing evidence, and reopen criteria.
...
- Broad `closure` or `negative-result` notes must cite a false-negative review or explicitly state which evidence categories remain incomplete.
