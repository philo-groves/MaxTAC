...
1. **Choose the auditor**: prefer the active domain pack's auditor MCP tools, such as Web, Binary, Supply Chains, Apple Systems, or Microsoft Systems auditor list/filter/show tools. If no domain pack provides a suitable auditor, Core includes a small fallback catalog in `<skill-dir>/references/auditors.json`. Do not read the JSON file directly; instead, run:
