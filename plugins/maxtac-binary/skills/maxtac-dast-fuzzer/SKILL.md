...
description: "Use this skill when binary or systems dynamic testing needs fuzzing for parsers, native libraries, kernels, protocols, managed runtimes, coverage-guided engines, grammar fuzzers, or harness selection."
---
...
Attach a reproduced crash or logic result:
...
3. Stateful protocol fuzzer with an explicit model of authentication, resource creation, and cleanup.
4. Request, content, or UI fuzzing only when lower-level harnessing is unavailable and the target is a systems protocol or native UI boundary.
...
- UI fuzzing when the vulnerable parser or protocol handler can be reached directly.
...
- **Generator**: Choose mutation, grammar, schema, protobuf, stateful protocol sequence, or UI event generation based on what the target accepts.
...
Use `maxtac-dast-debugger` for crash replay and runtime instrumentation.
