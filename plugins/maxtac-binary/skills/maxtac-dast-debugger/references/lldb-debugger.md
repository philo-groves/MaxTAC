...
Use LLDB for native runtime debugging, especially macOS, C, C++, and
LLVM/Clang-built targets. Prefer the installed `help`
...
If attach fails on Apple platforms, preserve the failure output and host state.
Do not reinterpret a debugger attach denial as target application behavior.
...
For macOS, the remote stub is typically `debugserver` through Apple tooling.
